...
| 49  | **Task Complexity Budget / Decomposition Gate**：核發前必須把施工 Task 降到目標模型可可靠執行的複雜度。`low` 可直接核發；`medium` 必須具備完整 Algorithm、邊界與 Verification；`high` 預設阻斷核發，必須先拆成 `low/medium` 子節點。只有人類明確指定高能力模型、填寫不可再拆原因並核准 override 時才可例外核發。**✅ 第一階段已完成**：純函式決策矩陣已移至 canonical workflow 資產、同步至生成資產，Task `task_complexity_policy@v1@d61303` 已通過 CP-2 並 deployed；完整 pytest 82 項通過。**尚待第二階段**：整合至 Task 核發 gate。 | ADAD Task Readiness + Kernel 核發 gate | **高**——原子節點不等於低複雜度，避免規格完整但弱模型仍無法可靠施工 |
