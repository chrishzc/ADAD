...
| 49  | **Task Complexity Budget / Decomposition Gate**：核發前必須把施工 Task 降到目標模型可可靠執行的複雜度。`low` 可直接核發；`medium` 必須具備完整 Algorithm、邊界與 Verification；`high` 預設阻斷核發，必須先拆成 `low/medium` 子節點。只有人類明確指定高能力模型、填寫不可再拆原因並核准 override 時才可例外核發。**✅ 第一階段已完成**：純函式決策矩陣已移至 canonical workflow 資產、同步至生成資產，Task `task_complexity_policy@v1@d61303` 已通過 CP-2 並 deployed；合併後完整 pytest 90 項通過。**尚待第二階段**：整合至 Task 核發 gate。 | ADAD Task Readiness + Kernel 核發 gate | **高**——原子節點不等於低複雜度，避免規格完整但弱模型仍無法可靠施工 |
...
|#|代辦事項|歸屬|優先度|對應第 1 節能力|
|---|---|---|---|---|
|51|`report_blocked` MCP server 尚未實作：需要結構化 blocked 理由、Task status 寫入、`blocked_report.schema.json`，以及不支援 MCP 平台的文字 parser 備援。此項原在 main 規格中使用 #48，為避免與 development 的「單一 `.venv`」#48 衝突，合併後改編為 #51。|ADAD 定義格式 + Kernel 提供掛勾點|高|1-4 卡住結構化回報|
...
1. **#49 第二階段**——把既有 Task Complexity Policy 接進 `generate_task` 核發 gate。
2. **#48 單一 `.venv`**——統一 CLI、hook 與專案環境生命週期。
3. **#51 `report_blocked`**——補齊迴圈內的結構化卡住回報能力。
4. **#3／#6／#11／#12／#15／#16／#17／#18／#32／#40／#41／#42 已完成並合併**。
