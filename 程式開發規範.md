...
### 🛠️ Skill 5: `check_invariants(node_name, file_path)`

- **功能：** 架構不變量與依賴邊界靜態檢查。
- **行為：** 讀取指定節點在 `system_map.yaml` 中定義的 `invariants` 規則（如 `deny_imports: [x]`）。透過解析目標代碼檔案的 AST，比對是否有違反規則的非法 import，如有則自動阻斷並回傳詳細 violation 報告，確保開發過程不會破壞既定架構理念（如分層隔離、防腐界限）。
