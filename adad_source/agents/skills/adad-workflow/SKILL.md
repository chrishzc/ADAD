...
Task ≠ Module。Module（`system_map.md`/`.yaml`）是架構長期存在的事實；Task 是**針對某一輪施工，正式核發給 coding 端的一份凍結快照**，位於 `.agents/tasks/<node_name>.task.json`，內容包含 Module 的完整 spec（含相依模組的 `invariants`，不只是 input/output 型別），外加 `task_id`、`source_hash`（偵測快照有沒有過期）與 `status`。
