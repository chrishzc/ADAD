...
* **支援四種規則，可在同一節點混用**：
  ...
  - `command`：單一步驟 CLI 驗證，例如 `command: {"argv": ["{python}", "{source}", "--check"], "expect_exit": "nonzero"}`。
  - `integration_case`：先將 `fixtures` 複製到隔離暫存工作區，再依序執行 `steps`；任一步失敗即停止。適合 migration 的 check／apply／資料檢查／idempotent 流程。
  - CLI 僅接受 argv 陣列並以 `shell=False` 執行。支援 `{python}`、`{source}`、`{project}`、`{workspace}`；`expect_exit` 可為整數或 `"nonzero"`，另可設定 `timeout`、`expect_stdout_contains`、`expect_stderr_contains`。fixture 只能從專案內複製到暫存工作區內。
