...
1. **定位目標檔案**：當你準備新增、修改或刪除某個模組（Module）或子系統（Subsystem）的架構時，**優先檢查其是否屬於某個子目錄模組**。
2. **追蹤 Include 鏈**：你必須先讀取根目錄的 `system_map.md`，確認你要變更的區塊是否已被 `<!-- include path/to/subfolder_map.md -->` 引入。若是，請直接編輯對應的子地圖 Markdown 檔案，而非根目錄檔案。
