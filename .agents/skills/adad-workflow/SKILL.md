...
### 1.5 🚦 PreToolUse Gate（動手改代碼之前就攔截，Claude Code 專用）
* **腳本**：`.agents/skills/adad-workflow/scripts/adad_pretooluse_gate.py`
* **這在解決什麼問題**：`adad_pre_commit.py`（見第 10 節）只在 `git commit` 那一刻才檢查狀態門禁，這代表 Agent 完全可能先花掉大量 token 把整支檔案寫完，才在 commit 時被擋下來、被迫重寫一次。這支腳本改掛在 Claude Code 的 **PreToolUse hook** 上，在 `Edit`/`Write`/`MultiEdit` 工具呼叫**執行前**就攔截：目標檔案對應的模組若處於不允許改代碼的狀態（例如 `pending_review` 以上），直接擋下這次工具呼叫並回饋明確原因，Agent 連一行都還沒寫出來就知道要先做什麼——這是機械層級的硬規則，不是自律，且 Claude Code 的 PreToolUse deny 判定發生在權限系統之前，`--dangerously-skip-permissions` 也繞不過去。
* **範圍與限制（誠實說明，不要誤解成萬能）**：目前只覆蓋「狀態門禁（RULE-02）」「禁止直接手改 `system_map.yaml`（該檔案是 `compile_map.py` 的編譯產物）」「Staleness 過期檢查」三項；**還沒有**檢查「這次修改前有沒有先執行 `read_context.py`」或「有沒有先呈報 CP-2 給人類審查」——也就是說，只要模組狀態允許修改（`planned`/`draft`/`dirty`/`validated` 都算），Agent 仍然可以不讀 context、不呈報計畫就直接動手改，這支腳本不會擋。它解決的是「明知故犯的狀態違規」，不是「跳過計畫呈報」，兩者是不同層次的問題，後者仍需 Agent 自律配合本 SKILL 的行為規範。
