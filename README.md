...
- **pytest 外層暫存根生命週期**：Verification runner 在 OS temp 建立 exclusive owned root，並將內層 `pytest-basetemp` 交給 pytest；外部顯式 `--basetemp` 維持 unowned，不會被 runner 清理。
- **Fail-closed 安全清理**：清理前逐層檢查 repo 邊界、symlink、Windows junction／reparse point 與目錄 identity；任何判斷或清理不確定時保留 workspace 並回報診斷路徑。
- **一致的結構化結果**：多 command 與 integration verification 會彙整最終 cleanup 狀態、`workspace_preserved` 與 `manual_action_required`，避免外層清理失敗時仍誤報成功。
